...
| P1 | Run `d=2,4,6,8` and `Delta=0,1,2,5,10,20` topology generation, then independent HPWL closure | M336-131 closes the first d6 rank layer; enumerate no-goods or find a strict certified improvement |
...
M336-131 reused the exact M336-122 d6/Delta20 domain with the certified M336-130
d6 hint. All hint distances were zero, and the formerly `UNKNOWN` model proved
rank 11 `OPTIMAL` in 14.332 DT. Independent one-opt/pair closure returned the
result to the M336-129 equal-HPWL swap plateau, so M336-118 remains the scoring
incumbent. Add the rank-11 tuple as a no-good and enumerate the next d6 layer;
